//
//  MahaSegmentedIndicatorBaseView.swift
//  MahaSegmentedView
//
...
public enum MahaSegmentedIndicatorPosition {
    case top
    case bottom
    case center
}

open class MahaSegmentedIndicatorBaseView: UIView, MahaSegmentedIndicatorProtocol {
    /// 默认MahaSegmentedViewAutomaticDimension（与cell的宽度相等）。内部通过getIndicatorWidth方法获取实际的值
    open var indicatorWidth: CGFloat = MahaSegmentedViewAutomaticDimension
    ...
    /// 默认MahaSegmentedViewAutomaticDimension（与cell的高度相等）。内部通过getIndicatorHeight方法获取实际的值
    open var indicatorHeight: CGFloat = MahaSegmentedViewAutomaticDimension
    /// 默认MahaSegmentedViewAutomaticDimension （等于indicatorHeight/2）。内部通过getIndicatorCornerRadius方法获取实际的值
    open var indicatorCornerRadius: CGFloat = MahaSegmentedViewAutomaticDimension
    /// 指示器的颜色
    open var indicatorColor: UIColor = .red
    /// 指示器的位置，top、bottom、center
    open var indicatorPosition: MahaSegmentedIndicatorPosition = .bottom
    /// 垂直方向偏移，指示器默认贴着底部或者顶部，verticalOffset越大越靠近中心。
    open var verticalOffset: CGFloat = 0
    /// 手势滚动、点击切换的时候，是否允许滚动。
    open var isScrollEnabled: Bool = true
    /// 是否需要将当前的indicator的frame转换到cell。辅助MahaSegmentedTitleDataSourced的isTitleMaskEnabled属性使用。
    ...
    ///  指示器的宽度是否跟随item的内容变化（而不是跟着cell的宽度变化）。indicatorWidth=MahaSegmentedViewAutomaticDimension才能生效
    ...
    public func getIndicatorHeight(itemFrame: CGRect) -> CGFloat {
        if indicatorHeight == MahaSegmentedViewAutomaticDimension {
    ...
    open func refreshIndicatorState(model: MahaSegmentedIndicatorSelectedParams) {
    }

    open func contentScrollViewDidScroll(model: MahaSegmentedIndicatorTransitionParams) {
    }

    open func selectItem(model: MahaSegmentedIndicatorSelectedParams) {
    }
}
