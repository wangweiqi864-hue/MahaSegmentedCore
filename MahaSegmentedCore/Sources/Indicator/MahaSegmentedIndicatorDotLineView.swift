...
open class MahaSegmentedIndicatorDotLineView: MahaSegmentedIndicatorBaseView {
    ...
    open override func contentScrollViewDidScroll(model: MahaSegmentedIndicatorTransitionParams) {
        ...
        if percent <= 0.5 {
            targetX = MahaSegmentedViewTool.interpolate(from: leftX, to: centerX, percent: CGFloat(percent*2))
            targetWidth = MahaSegmentedViewTool.interpolate(from: dotWidth, to: lineMaxWidth, percent: CGFloat(percent*2))
        }else {
            targetX = MahaSegmentedViewTool.interpolate(from: centerX, to: rightX, percent: CGFloat((percent - 0.5)*2))
            targetWidth = MahaSegmentedViewTool.interpolate(from: lineMaxWidth, to: dotWidth, percent: CGFloat((percent - 0.5)*2))
        }
