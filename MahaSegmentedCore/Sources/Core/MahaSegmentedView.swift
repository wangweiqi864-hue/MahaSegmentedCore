...
public let MahaSegmentedViewAutomaticDimension: CGFloat = -1
...
/// - click: 通过点击item选中
/// - scroll: 通过滚动到item选中
public enum MahaSegmentedViewItemSelectedType {
    case unknown
    case code
    case click
    case scroll
...
public protocol MahaSegmentedViewListContainer {
...
public protocol MahaSegmentedViewDataSource: AnyObject {
    ...
    /// 返回数据源数组，数组元素必须是MahaSegmentedBaseItemModel及其子类
    ///
    /// - Parameter segmentedView: MahaSegmentedView
    /// - Returns: 数据源数组
    func itemDataSource(in segmentedView: MahaSegmentedView) -> [MahaSegmentedBaseItemModel]

    /// 返回index对应item的宽度，等同于cell的宽度。
    ///
    /// - Parameters:
    ///   - segmentedView: MahaSegmentedView
    ///   - index: 目标index
    /// - Returns: item的宽度
    func segmentedView(_ segmentedView: MahaSegmentedView, widthForItemAt index: Int) -> CGFloat
    ...
    ///   - segmentedView: MahaSegmentedView
    ///   - index: 目标index
    func segmentedView(_ segmentedView: MahaSegmentedView, widthForItemContentAt index: Int) -> CGFloat

    /// 注册cell class
    ///
    /// - Parameter segmentedView: MahaSegmentedView
    func registerCellClass(in segmentedView: MahaSegmentedView)
    ...
    ///   - segmentedView: MahaSegmentedView
    ///   - index: 目标index
    /// - Returns: MahaSegmentedBaseCell及其子类
    func segmentedView(_ segmentedView: MahaSegmentedView, cellForItemAt index: Int) -> MahaSegmentedBaseCell

    /// 根据当前选中的selectedIndex，刷新目标index的itemModel
    ///
    /// - Parameters:
    ///   - itemModel: MahaSegmentedBaseItemModel
    ///   - index: 目标index
    ///   - selectedIndex: 当前选中的index
    func refreshItemModel(_ segmentedView: MahaSegmentedView, _ itemModel: MahaSegmentedBaseItemModel, at index: Int, selectedIndex: Int)
    ...
    ///   - selectedType: 选中的类型
    func refreshItemModel(_ segmentedView: MahaSegmentedView, currentSelectedItemModel: MahaSegmentedBaseItemModel, willSelectedItemModel: MahaSegmentedBaseItemModel, selectedType: MahaSegmentedViewItemSelectedType)
    ...
    ///   - percent: 从左到右的百分比
    func refreshItemModel(_ segmentedView: MahaSegmentedView, leftItemModel: MahaSegmentedBaseItemModel, rightItemModel: MahaSegmentedBaseItemModel, percent: CGFloat)
}

/// 为什么会把选中代理分为三个，因为有时候只关心点击选中的，有时候只关心滚动选中的，有时候只关心选中。所以具体情况，使用对应方法。
public protocol MahaSegmentedViewDelegate: AnyObject {
    /// 点击选中或者滚动选中都会调用该方法。适用于只关心选中事件，而不关心具体是点击还是滚动选中的情况。
    ///
    /// - Parameters:
    ///   - segmentedView: MahaSegmentedView
    ///   - index: 选中的index
    func segmentedView(_ segmentedView: MahaSegmentedView, didSelectedItemAt index: Int)
    ...
    ///   - segmentedView: MahaSegmentedView
    ///   - index: 选中的index
    func segmentedView(_ segmentedView: MahaSegmentedView, didClickSelectedItemAt index: Int)
    ...
    ///   - segmentedView: MahaSegmentedView
    ///   - index: 选中的index
    func segmentedView(_ segmentedView: MahaSegmentedView, didScrollSelectedItemAt index: Int)
    ...
    ///   - percent: 从左往右计算的百分比
    func segmentedView(_ segmentedView: MahaSegmentedView, scrollingFrom leftIndex: Int, to rightIndex: Int, percent: CGFloat)
    ...
    ///   - segmentedView: MahaSegmentedView
    ///   - index: 目标index
    func segmentedView(_ segmentedView: MahaSegmentedView, canClickItemAt index: Int) -> Bool
}

/// 提供MahaSegmentedViewDelegate的默认实现，这样对于遵从MahaSegmentedViewDelegate的类来说，所有代理方法都是可选实现的。
public extension MahaSegmentedViewDelegate {
    func segmentedView(_ segmentedView: MahaSegmentedView, didSelectedItemAt index: Int) { }
    func segmentedView(_ segmentedView: MahaSegmentedView, didClickSelectedItemAt index: Int) { }
    func segmentedView(_ segmentedView: MahaSegmentedView, didScrollSelectedItemAt index: Int) { }
    func segmentedView(_ segmentedView: MahaSegmentedView, scrollingFrom leftIndex: Int, to rightIndex: Int, percent: CGFloat) { }
    func segmentedView(_ segmentedView: MahaSegmentedView, canClickItemAt index: Int) -> Bool { return true }
}

/// 内部会自己找到父UIViewController，然后将其automaticallyAdjustsScrollViewInsets设置为false，这一点请知晓。
open class MahaSegmentedView: UIView, MahaSegmentedViewRTLCompatible {
    open weak var dataSource: MahaSegmentedViewDataSource? {
        didSet {
            dataSource?.reloadData(selectedIndex: selectedIndex)
        }
    }
    open weak var delegate: MahaSegmentedViewDelegate?
    open private(set) var collectionView: MahaSegmentedCollectionView!
    ...
    public var listContainer: MahaSegmentedViewListContainer? = nil {
        didSet {
            listContainer?.defaultSelectedIndex = defaultSelectedIndex
            contentScrollView = listContainer?.contentScrollView()
        }
    }
    /// indicators的元素必须是遵从MahaSegmentedIndicatorProtocol协议的UIView及其子类
    ...
    /// 整体内容的左边距，默认MahaSegmentedViewAutomaticDimension（等于itemSpacing）
    open var contentEdgeInsetLeft: CGFloat = MahaSegmentedViewAutomaticDimension
    /// 整体内容的右边距，默认MahaSegmentedViewAutomaticDimension（等于itemSpacing）
    open var contentEdgeInsetRight: CGFloat = MahaSegmentedViewAutomaticDimension
    /// 点击切换的时候，contentScrollView的切换是否需要动画
    open var isContentScrollViewClickTransitionAnimationEnabled: Bool = true

    private var itemDataSource = [MahaSegmentedBaseItemModel]()
    ...
    private func commonInit() {
        ...
        collectionView = MahaSegmentedCollectionView(frame: CGRect.zero, collectionViewLayout: layout)
        collectionView.backgroundColor = .clear
        collectionView.showsVerticalScrollIndicator = false
        collectionView.showsHorizontalScrollIndicator = false
        collectionView.scrollsToTop = false
        collectionView.register(UICollectionViewCell.self, forCellWithReuseIdentifier: "MahaSegmentedViewInnerEmptyCell")
    ...
    open override func layoutSubviews() {
        ...
        //部分使用者为了适配不同的手机屏幕尺寸，MahaSegmentedView的宽高比要求保持一样。所以它的高度就会因为不同宽度的屏幕而不一样。计算出来的高度，有时候会是位数很长的浮点数，如果把这个高度设置给UICollectionView就会触发内部的一个错误。所以，为了规避这个问题，在这里对高度统一向下取整。
        //如果向下取整导致了你的页面异常，请自己重新设置MahaSegmentedView的高度，保证为整数即可。
    ...
    //MARK: - Public
    public final func dequeueReusableCell(withReuseIdentifier identifier: String, at index: Int) -> MahaSegmentedBaseCell {
        let indexPath = IndexPath(item: index, section: 0)
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: identifier, for: indexPath)
        guard cell.isKind(of: MahaSegmentedBaseCell.self) else {
            fatalError("Cell class must be subclass of MahaSegmentedBaseCell")
        }
        return cell as! MahaSegmentedBaseCell
    }
...
//        }
    ...
    open func reloadDataWithoutListContainer() {
        ...
        if dataSource?.isItemSpacingAverageEnabled == true && totalContentWidth < bounds.size.width {
            ...
            if contentEdgeInsetLeft == MahaSegmentedViewAutomaticDimension {
                itemSpacingCount += 1
            }else {
                totalItemSpacingWidth -= contentEdgeInsetLeft
            }
            if contentEdgeInsetRight == MahaSegmentedViewAutomaticDimension {
                itemSpacingCount += 1
            }else {
        ...
        if contentScrollView != nil {
            if contentScrollView!.frame.equalTo(CGRect.zero) &&
                ...
                //某些情况系统会出现MahaSegmentedView先布局，contentScrollView后布局。就会导致下面指定defaultSelectedIndex失效，所以发现contentScrollView的frame为zero时，强行触发其父视图链里面已经有frame的一个父视图的layoutSubviews方法。
                //比如MahaSegmentedListContainerView会将contentScrollView包裹起来使用，该情况需要MahaSegmentedListContainerView.superView触发布局更新
        ...
        for indicator in indicators {
            if itemDataSource.isEmpty {
                ...
                let indicatorParams = MahaSegmentedIndicatorSelectedParams(currentSelectedIndex: selectedIndex,
    ...
    open func reloadItem(at index: Int) {
        ...
        let cell = collectionView.cellForItem(at: IndexPath(item: index, section: 0)) as? MahaSegmentedBaseCell
    ...
    open override func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?) {
        if keyPath == "contentOffset" {
            ...
            if contentScrollView?.isTracking == true || contentScrollView?.isDecelerating == true {
                ...
                let indicatorParams = MahaSegmentedIndicatorTransitionParams(currentSelectedIndex: selectedIndex,
                                                                           leftIndex: baseIndex,
                ...
                if remainderProgress == 0 {
                    ...
                    let leftCell = collectionView.cellForItem(at: IndexPath(item: baseIndex, section: 0)) as? MahaSegmentedBaseCell
                    leftCell?.reloadData(itemModel: itemDataSource[baseIndex], selectedType: .unknown)

                    let rightCell = collectionView.cellForItem(at: IndexPath(item: baseIndex + 1, section: 0)) as? MahaSegmentedBaseCell
    ...
    private func selectItemAt(index: Int, selectedType: MahaSegmentedViewItemSelectedType) {
        guard index >= 0 && index < itemDataSource.count else {
            return
        }
        ...
        let currentSelectedCell = collectionView.cellForItem(at: IndexPath(item: selectedIndex, section: 0)) as? MahaSegmentedBaseCell
        currentSelectedCell?.reloadData(itemModel: currentSelectedItemModel, selectedType: selectedType)

        let willSelectedCell = collectionView.cellForItem(at: IndexPath(item: index, section: 0)) as? MahaSegmentedBaseCell
        ...
        if scrollingTargetIndex != -1 && scrollingTargetIndex != index {
            ...
            let scrollingTargetCell = collectionView.cellForItem(at: IndexPath(item: scrollingTargetIndex, section: 0)) as? MahaSegmentedBaseCell
...
extension MahaSegmentedView: UICollectionViewDataSource {
    ...
    public func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        if let cell = dataSource?.segmentedView(self, cellForItemAt: indexPath.item) {
            ...
            return collectionView.dequeueReusableCell(withReuseIdentifier: "MahaSegmentedViewInnerEmptyCell", for: indexPath)
        }
    }
}

extension MahaSegmentedView: UICollectionViewDelegate {
...
extension MahaSegmentedView: UICollectionViewDelegateFlowLayout {
